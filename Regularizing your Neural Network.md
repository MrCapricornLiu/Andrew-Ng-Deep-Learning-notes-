...
$J(w,b)=\frac{1}{m}\Sigma^{m}_{i=1}L(\^y^{(i)},y^{(i)})$
>**J**:Cost function  **L**:Loss function

$J(w,b)=\frac{1}{m}\Sigma^{m}_{i=1}L(\^y^{(i)},y^{(i)})+\frac{\lambda}{2m}||w||_{2}^{2}$
...
$J(w^{[1]},b^{[1]},\cdots,w^{[L]},b^{[L]})=\frac{1}{m}\Sigma^{m}_{i=1}L(\^y^{(i)},y^{(i)})+\frac{\lambda}{2m}\Sigma^{L}_{l=1}||w^{[l]}||_{F}^{2}$
